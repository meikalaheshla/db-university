...
SELECT `degrees`.`id`, `degrees`.`name`, `courses`.`name`, `teachers`.`id`, `teachers`.`name`, `teachers`.`surname`
FROM `degrees`
JOIN `courses`
...
JOIN `departments` 
ON `departments`.`id` = `degrees`.`department_id`
WHERE `departments`.`id` = 54 OR `departments`.`name` = 'Dipartimento di Matematica';
